  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", CubismMaskRenderer = class CubismMaskRenderer {
        ...
        get meshBounds() {
          var mesh = this._mainRenderer.mesh;
        ...
        addToCommandBuffer(buffer) {
          ...
          var sym = (_crd && CubismMaskCommandBuffer === void 0 ? (_reportPossibleCrUseOfCubismMaskCommandBuffer({
            error: Error()
          }), CubismMaskCommandBuffer) : CubismMaskCommandBuffer).metadataPropertySymbol;

          function findRenderer(renderers) {
            for (var i = 0; i < renderers.length; i++) {
              var _meta = Reflect.get(renderers[i], sym);

              if (_meta && _meta.cubismRenderer == mainRenderer) {
                return _meta;
              }
            }
          ...
          var maskRenderers = (_crd && CubismMaskCommandBuffer === void 0 ? (_reportPossibleCrUseOfCubismMaskCommandBuffer({
            error: Error()
          }), CubismMaskCommandBuffer) : CubismMaskCommandBuffer).instance.getComponentsInChildren(MeshRenderer);
          var meta = findRenderer(maskRenderers);
          ...
          if (!meta) {
            ...
            var isCulling = !((_mainRenderer$node$ge = mainRenderer.node.getComponent(_crd && CubismDrawable === void 0 ? (_reportPossibleCrUseOfCubismDrawable({
              error: Error()
            }), CubismDrawable) : CubismDrawable)) != null && _mainRenderer$node$ge.isDoubleSided);
            var node = new Node(mainRenderer.node.name);
            ...
            var _maskMaterial = isCulling ? this.maskMaterial : this.maskMaterial;

            meta.meshRenderer.setMaterial(_maskMaterial, 0);
          ...
          var maskRenderer = meta.meshRenderer;
          var maskMaterial = maskRenderer.getMaterial(0);
