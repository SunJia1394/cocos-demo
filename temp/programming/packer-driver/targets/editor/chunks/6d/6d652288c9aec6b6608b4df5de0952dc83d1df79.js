  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", CubismPartsInspector = (_dec = _decorator.ccclass('CubismPartsInspector'), _dec(_class = class CubismPartsInspector extends Component {
         ...
         */
        getParts() {
          const cubismModel = (_crd && ComponentExtensionMethods === void 0 ? (_reportPossibleCrUseOfComponentExtensionMethods({
            error: Error()
          ...
          const parts = new Array(partsSource.length);

          for (let i = 0; i < partsSource.length; i++) {
            let displayName;
            const diPartName = partsSource[i].getComponent(_crd && CubismDisplayInfoPartName === void 0 ? (_reportPossibleCrUseOfCubismDisplayInfoPartName({
              error: Error()
