    ...
    execute: function () {
      ...
      _export("default", CubismExpressionController = (_dec = ccclass('CubismExpressionController'), _dec2 = property({
        ...
        startExpression() {
          ...
          const palyingExpression = CubismPlayingExpression.create(this._model, expressionsList.cubismExpressionObjects[this.currentExpressionIndex]);
        ...
        onLateUpdate() {
          ...
          for (let expressionIndex = 0; expressionIndex < this._playingExpressions.length; ++expressionIndex) {
            ...
            const fadeIn = Math.abs(playingExpression.fadeInTime) < math.EPSILON ? 1.0 : CubismFadeMath.getEasingSine(playingExpression.expressionUserTime / playingExpression.fadeInTime);
            const fadeOut = Math.abs(playingExpression.expressionEndTime) < math.EPSILON || playingExpression.expressionEndTime < 0.0 ? 1.0 : CubismFadeMath.getEasingSine((playingExpression.expressionEndTime - playingExpression.expressionUserTime) / playingExpression.fadeOutTime);
